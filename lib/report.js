...
const isStatisticallyFaster = (previous, current) => {
  return current.mean + current.stddev < previous.mean - previous.stddev && current.mean / previous.mean < 0.90
}

const isStatisticallySlower = (previous, current) => {
  return current.mean - current.stddev > previous.mean + previous.stddev && previous.mean / current.mean < 0.90
}
